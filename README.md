...
To also create one PDF per chapter, enable `per-chapter`. Chapter filenames use the chapter names from `SUMMARY.md`; selection uses source paths from `SUMMARY.md`. If `print-chapters` is set, only those chapters are printed; a chapter in both lists is still printed.
